...
function print_logs(){
    service=$1
    tail=$2
    service_name=$([ "$service" = "db_docker" ] && echo "Base de datos" || echo "Aplicacion")
...
function print_status(){
    ...
    for service in "db_docker" "app_docker" 
    do
        service_name=$([ "$service" = "db_docker" ] && echo "Base de datos" || echo "Aplicacion")
...
do
    ...
    if [ "$option" = "$option_1" ]; then
        ...
        if [ -f "./db_docker/mongo_replication.key" ]; then  
            # Si existe el archivo verificamos que el comando 'chown' se haya ejecutado correctamente 
            file_owner=$(ls -l ./db_docker/mongo_replication.key | awk '{print $3}')
            file_group=$(ls -l ./db_docker/mongo_replication.key | awk '{print $4}')
            owner=$(id -nu 999)
            group=$(id -ng 999)
            if [[ ! "$file_owner" = "$owner" ]] && [[ ! "$file_group" = "$group" ]]; then
                rm -f ./db_docker/mongo_replication.key
            fi
        fi

        # Iniciamos/Detenemos el servicio de base de datos
        res=$(is_active "db_docker")
        if [ "$res" -eq 0 ]; then
            if ! [ -f "./db_docker/mongo_replication.key" ]; then  
                # Si no existe el archivo entonces lo creamos
                openssl rand -base64 768 > ./db_docker/mongo_replication.key
                chmod 400 ./db_docker/mongo_replication.key
                # Se crea un bucle hasta que el usuario introduzca la contrasena correcta
                sudo chown 999:999 ./db_docker/mongo_replication.key
            fi

            # Iniciamos el servicio de base de datos
            docker-compose -f ./db_docker/docker-compose.yaml --env-file ./db_docker/.env up -d
        else
            # Detenemos el servicio de base de datos
            docker-compose -f ./db_docker/docker-compose.yaml --env-file ./db_docker/.env down
        fi
